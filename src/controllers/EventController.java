...
public class EventController {
    ...
    public static boolean cancelEvent(int userId, String eventId) {
        System.out.println("Attempting to cancel event: " + eventId + " for user: " + userId);
        String query = "DELETE FROM bookings WHERE user_id = ? AND event_id = ?";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setInt(1, userId);
